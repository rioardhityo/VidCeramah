...
public class ustadSatu extends AppCompatActivity {
    ...
    private void parsingData(String types) {
        ...
        SampleAPI.Factory.getIstance(ustadSatu.this).getData(url).enqueue(new Callback<ModelVid>() {
            @Override
            public void onResponse(Call<ModelVid> call, Response<ModelVid> response) {
                if (response.isSuccessful()) {
                    datanya1 = response.body().getItems();
                    adapter = new itemlist(ustadSatu.this, datanya1);
            ...
            public void onFailure(Call<ModelVid> call, Throwable t) {
                //pd.dismiss();
                Toast.makeText(ustadSatu.this, "Error : " + t.getMessage(), Toast.LENGTH_LONG).show();
