...
public class ustadTiga extends AppCompatActivity {
    ...
    private void parsingData(String types) {
        ...
        SampleAPI.Factory.getIstance(ustadTiga.this).getData(url).enqueue(new Callback<ModelVid>() {
            @Override
            public void onResponse(Call<ModelVid> call, Response<ModelVid> response) {
                if (response.isSuccessful()) {
                    datanya1 = response.body().getItems();
                    adapter = new itemlist(ustadTiga.this, datanya1);
                    rv.setAdapter(adapter);
                    rv.invalidate();
                    pd.dismiss();
                    //Toast.makeText(ustadTiga.this, "Say Hello to papa !", Toast.LENGTH_SHORT).show();
                } else {
                    Toast.makeText(ustadTiga.this, "Kenapa bro : " + response.message() + response.errorBody(), Toast.LENGTH_SHORT).show();
                }
            ...
            public void onFailure(Call<ModelVid> call, Throwable t) {
                //pd.dismiss();
                Toast.makeText(ustadTiga.this, "Error : " + t.getMessage(), Toast.LENGTH_LONG).show();
